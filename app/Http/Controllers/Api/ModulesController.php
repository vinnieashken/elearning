...
//            }
        ...
        $dates = Choiceless::where('user_id',$userid)->whereIn('module_id',$choiceless)->get(['module_id','created_at']);
        ...
        foreach ($choiceless as $item)
        {
            ...
            {
                if($date->module_id == $item->id)
                {
                    $item->date = $date->created_at;
                }
            }
            $sheets->push($item);
        }
    ...
    public function ModulesAverage(Request $request)
    {
        if($request->has('page') && $request->has('size'))
        {
            ...
            $scores = AnswerSheet::where('module_id',10)
                ->leftjoin('user_answers','user_answers.question_id','=','answers.question_id')
                ->count();
